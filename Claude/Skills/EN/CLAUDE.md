...
<!-- SOPIFY_VERSION: 2026-03-20.141842 -->
...
Note: before every Sopify LLM round, the host must run the long-term preference preload first; discover the helper from `.sopify-runtime/manifest.json -> limits.preferences_preload_entry` and fall back to `scripts/preferences_preload_runtime.py inspect --workspace-root <cwd>` only in repo-local development mode. The host must consume the helper's JSON contract only; inject `injection_text` only when top-level `status == ready` and `preferences.status == loaded` and `injected == true`, and treat `missing / invalid / read_error` as `fail-open with visibility`.
...
- Before every new Sopify LLM round, the host must run the long-term preference preload; this includes fresh requests, clarification/decision/execution-confirm resumes, and ordinary continuation into the next LLM turn.
- Discover the long-term preference preload helper from `.sopify-runtime/manifest.json -> limits.preferences_preload_entry`; fall back to `scripts/preferences_preload_runtime.py` only when the vendored helper is unavailable in repo-local development mode.
- Consume only the stable JSON contract returned by the helper; inject `preferences.injection_text` only when `status == ready` and `preferences.status == loaded` and `preferences.injected == true`, and never re-read `preferences.md` to rebuild the prompt block manually.
