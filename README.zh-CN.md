...
[![版本](https://img.shields.io/badge/version-2026--05--28.044700-orange.svg)](#版本历史)
...
用户输入经过宿主适配器（Codex、Claude、Copilot）进入核心协议层，每个操作都经历提议、校验、闸门、收据四步。Validator 是唯一授权者 — 宿主 LLM 只是提议来源。知识层（蓝图、方案、历史）跨 session 和宿主持久保留。
...
当你修改 `skills/{zh,en}`（源模板或 skill 包），或修改 `runtime/builtin_skill_packages/*/skill.yaml` 时，按 [贡献指南](./CONTRIBUTING_CN.md) 跑同步与校验命令。
